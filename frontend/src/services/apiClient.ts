...
// Use '/api' prefix in production (Docker/Nginx)
...
export const getRecommendations = async (request: BookRecommendationRequest) => {
  try {
    ...
    // Ensure we're always returning a valid response structure
    if (!response.data) {
      return { recommendations: [] };
    }

    // If the response already has recommendations property and it's an array
    ...
    // If it's an object but doesn't have recommendations array
    return { 
      recommendations: [{ 
